...
def graphDBQuery(context, query, traversalDepth = 1, maxEdges = -1):
    ...
    if "valueMap" in query:
        raise Exception("There is no need for valueMap() as it is performed automatically.")
        return
    ...
    if canValueMap:
        ...
        if isEQuery:
            ...
            iterators = [ iterator1.outV(), iterator2.inV() ]
        else:
...
def getAPIResponseGenerator(results):
    def generate(results):
        ...
        yield getAPIJSONResponseFooter()
    return StreamingResponse(generate(results), media_type="application/json")
...
def getAPIErrorResponseGenerator(e):
    def generate(e):
        ...
        yield libutil.toJSON(getAPIErrorResponse(e))
        yield getAPIJSONResponseFooter()
    return StreamingResponse(generate(e), media_type="application/json")
